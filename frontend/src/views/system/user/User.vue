    ...
    <div>
      ...
      <a-table ref="TableInfo"
        ...
        <template slot="operation" slot-scope="text, record">
          ...
          &nbsp;
          <a-icon type="alert" theme="twoTone" twoToneColor="#42b983" @click="view(record)" title="发送"></a-icon>
...
export default {
  ...
  methods: {
    ...
    view (record) {
      if (record.email) {
        this.$message.loading('正在发送', 0)
        this.$get('/cos/consume/send', {userId: record.userId}).then((r) => {
          this.$message.destroy()
          this.$message.success('发送成功')
        })
      } else {
        this.$message.error('该账户没有邮箱地址')
      }
    },
    add () {
