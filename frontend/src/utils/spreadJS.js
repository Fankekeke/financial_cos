...
const initSpread = (tableName, spread, callback) => {
  // 从本地读取JSON模板 导入到spread里
  request({
    url: `/cdn/spread/template/${fileNameData[tableName]}`,
    method: 'get'
  }).then(response => {
    spread.fromJSON(response.data)
...
export { newSpread, fixedForm, floatForm, floatReset, saveExcel, initSpread }
